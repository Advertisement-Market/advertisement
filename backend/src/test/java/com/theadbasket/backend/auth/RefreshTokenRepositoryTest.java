 ...
 * runs in production). Backdates one token's {@code created_ts} past the cutoff and asserts only it
 * is removed.
 */
@DataJpaTest
...
class RefreshTokenRepositoryTest {
    ...
    void deletesOnlyTokensOlderThanCutoff() {
        ...
        // created_ts is @CreatedDate (set to now on persist); backdate the old token past retention.
        em.getEntityManager()
                .createNativeQuery("update refresh_tokens set created_ts = ?1 where id = ?2")
