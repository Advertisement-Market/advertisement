    ...
    <nav className="site-nav">
      <div className="nav-left">
        ...
        <div className="nav-switcher" ref={switcherRef}>
          ...
          />
          <Link to={ROUTES.browse} className="nav-switch-btn">
            Billboards
          </Link>
...
function AgencyCard({ a, index, onOpen }) {
  const serviceTags = a.services.slice(0, 4).map((s) => (
    <span key={s} className="card-service-tag">
      {s}
    </span>
  ));
  return (
    ...
    >
      <div className="card-img">
        ...
        <span className={cn('card-type-badge', TYPE_BADGE_CLASS[a.type] || 'tb-full')}>
          {a.type}
        </span>
      ...
      <div className="card-body">
        ...
        <div className="card-service-tags">
          {serviceTags}
          {a.services.length > 4 && (
            <span className="card-service-tag overflow">+{a.services.length - 4} more</span>
          )}
        </div>
        <div className="card-stats">
          <div className="card-stat">
            <div className="stat-num">{a.expYears}yr</div>
            <div className="stat-label">Experience</div>
          </div>
          <div className="card-stat">
            <div className="stat-num">{a.campaigns}</div>
            <div className="stat-label">Campaigns</div>
          </div>
          <div className="card-stat">
            <div className="stat-num">{a.industries.length}</div>
        ...
        <div className="dp-body">
          <div className="dp-tags">
            {a.services.slice(0, 3).map((s) => (
              <span key={s} className="dp-tag service-tag">
                {s}
              </span>
            ))}
          </div>
          ...
          <div className="dp-specs-grid">
            {[
              ['HQ', a.hq],
              ['Established', `${a.yearEst} (${a.expYears} yrs)`],
              ['Campaigns', a.campaigns],
              ['Min Budget', a.minBudget],
              ['Coverage', a.coverage],
          ...
          <div className="dp-tags">
            {a.industries.map((i) => (
              <span key={i} className="dp-tag industry-tag">
                {i}
              </span>
            ))}
          </div>
          <div className="dp-section-title">Operating Cities</div>
          <p style={{ fontSize: 13, color: 'var(--ink-muted)', fontWeight: 300, marginBottom: 6 }}>
            {a.cities}
          </p>
...
export function BrowseAgencies() {
  ...
  const filtered = useMemo(() => {
    ...
    let list = AGENCIES.filter((a) => {
      ...
      if (
        services.length &&
        !services.every((sv) => a.services.some((as) => as.includes(sv.split(' ')[0])))
      )
        return false;
      if (industries.length && !industries.every((i) => a.industries.some((ai) => ai.includes(i))))
        return false;
      if (s) {
        const h = [a.name, a.tagline, a.type, a.city, ...a.services, ...a.industries, a.clients]
          .join(' ')
          .toLowerCase();
    ...
    else if (sort === 'campaigns_desc')
      list = [...list].sort((a, b) => b.campaignsNum - a.campaignsNum);
    else if (sort === 'budget_asc')
      list = [...list].sort((a, b) => a.minBudgetNum - b.minBudgetNum);
    return list;
    ...
    <div className="browse-agencies-page">
      ...
      <div className="browse-bar">
        <div className="browse-search-wrap">
          ...
          <input
            ...
            placeholder="Agency name, service, city…"
            value={search}
        ...
        <div className="browse-bar-right">
          <Dropdown
            ...
            trigger={{
              hasValue: false,
              label: SORTS.find((s) => s.value === sort)?.label || 'Relevance',
              icon: (
                <svg
                  width="12"
                  height="12"
        ...
        <aside className="filter-sidebar">
          ...
          <div className="filter-section">
            ...
            <div className="sidebar-search-wrap">
              ...
              <input
                ...
                placeholder="Agency, service, city…"
                value={search}
                onChange={(e) => setSearch(e.target.value)}
              />
            </div>
          ...
          <div className="filter-section">
            ...
            <div className="filter-pills">
              {SERVICE_PILLS.map((p) => (
                <button
                  key={p.v}
                  className={cn('filter-pill', services.includes(p.v) && 'active')}
                  onClick={() => toggleService(p.v)}
                >
                  {p.l}
                </button>
              ))}
          ...
          <div className="filter-section">
            ...
            <div className="filter-pills">
              {INDUSTRY_PILLS.map((p) => (
                <button
                  key={p.v}
                  className={cn('filter-pill', industries.includes(p.v) && 'active')}
                  onClick={() => toggleIndustry(p.v)}
                >
                  {p.l}
                </button>
              ))}
          ...
          <div className="filter-section">
            ...
            <div className="filter-toggle-row">
              ...
              <div
                className={cn('toggle-switch', verifiedOnly && 'on')}
        ...
        <div className="browse-main">
          <div className="results-topbar">
            <div className="results-count">
              Showing <strong>{filtered.length ? `1–${filtered.length}` : '0'}</strong> of{' '}
              <strong>{filtered.length}</strong> agencies
            </div>
          </div>
          ...
          {chips.length > 0 && (
            <div className="active-filters-row" style={{ display: 'flex' }}>
              ...
              <button className="filter-reset-btn" style={{ marginLeft: 4 }} onClick={resetFilters}>
                Clear all
              </button>
            </div>
          )}
      ...
      {detail && (
        <DetailPanel
          ...
          onSubmit={() => {
            showToast('Inquiry sent — the agency will respond within 48 hours.');
            setDetailId(null);
          }}
        />
      )}
